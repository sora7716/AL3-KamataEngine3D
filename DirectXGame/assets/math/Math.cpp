...
// 反射ベクトル
Vector3 Math::ReflectVector(const Vector3& input, const Vector3& normal) { return input - 2 * Math::Dot(input, normal) * normal; }
...
Vector3 Math::Friction(const Ball& ball, float miu) {
	...
	if (abs(ball.velocity.x) > 0.01f || abs(ball.velocity.y) > 0.01f || abs(ball.velocity.z) > 0.01f) {
		...
		if (abs(frictionalForce.x * deltaTime) > abs(ball.velocity.x) || abs(frictionalForce.y * deltaTime) > abs(ball.velocity.y) || abs(frictionalForce.z * deltaTime) > abs(ball.velocity.z)) {
...
float Math::LerpShortAngle(float a, float b, float t) {
	...
	// 角度差分を [-PI, +PI] の範囲に補正
	diff = fmod(diff + pi_f, 2.0f * pi_f) - pi_f;
