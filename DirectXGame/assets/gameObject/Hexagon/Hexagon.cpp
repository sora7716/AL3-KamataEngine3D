...
void Hexagon::Initialize(Model* model, ViewProjection* viewProjection, MapChipField* mapChipField) {
	...
	float radius = MapChipField::kHexagonRadius;                // 半径
	worldTransforms_.resize(MapChipField::kNumBlockHorizontal); // 層の数を設定
	...
	for (int i = 0; i < MapChipField::kNumBlockHorizontal; i++) {
		worldTransforms_[i].resize(MapChipField::kNumBlockVertical);
		for (int j = 0; j < MapChipField::kNumBlockVertical; j++) {
			if (mapChipField_->GetMapChipTypeByIndex(j, i) == MapChipType::kBlock) {
				WorldTransform* worldTransform = new WorldTransform();
				worldTransform->Initialize();
				worldTransform->translation_.x = (j + 0.5f * (i % 2)) * radius * sqrt(3.0f);
				worldTransform->translation_.z = i * (1.5f * radius);
