...
void CameraController::Update() {
	...
	viewProjection_.translation_.x = std::clamp(
	    viewProjection_.translation_.x,
	    target_->GetWorldTransform().translation_.x + margin.left,
	    target_->GetWorldTransform().translation_.x + margin.right);

	viewProjection_.translation_.y = std::clamp(
	    viewProjection_.translation_.y,
	    target_->GetWorldTransform().translation_.y + margin.bottom,
	    target_->GetWorldTransform().translation_.y + margin.top);
	// プレイヤーが見切れないようにするためのもの↑
