...
void Player::MapChipRight(CollisionMapChipInfo& info) {
	...
	if (hit) {
		...
		info.velocity.y = max(0.0f, (rect.right - worldTransform_.translation_.x) - (velocity_.x + Blanc));
		//壁に当たっているかのフラグ
...
void Player::MapChipLeft(CollisionMapChipInfo& info) {
	...
	if (hit) {
		...
		info.velocity.y = min(0.0f, (rect.left - worldTransform_.translation_.x) + ((kWidth / 2.0f) + Blanc));
		//壁に当たっているかのフラグ
